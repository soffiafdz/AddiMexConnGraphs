...
A.norm.sub <- matsWB$A.norm.sub
A.norm.mean <- matsWB$A.norm.mean
atlas <- "power"
g.groupWB <- gWB <- fnames <- vector('list', length=length(groups))
...
for (i in seq_along(groups)) {
    ...
    g.groupWB[[i]] <- lapply(seq_along(thresholds), function(x) 
        graph_from_adjacency_matrix(A.norm.mean[[x]][[i]], 
                                    mode = 'undirected', diag = F, weighted = T))
    
    for (x in seq_along(thresholds)) {V(g.groupWB[[i]][[x]])$name <- as.character(power$name)}
    
    g.groupWB[[i]] <- llply(seq_along(thresholds), function(x) 
        set_brainGraph_attr_sof(g.groupWB[[i]][[x]], atlas, modality = 'fmri', 
...
for (i in seq_along(groups)) {
    gWB[[i]] <- fnames[[i]] <- vector('list', length = length(thresholds))
    for (j in seq_along(thresholds)) {
        fnames[[i]][[j]] <- list.files(savedir, 
                                       sprintf('*g%i_thr%02i.*', i, j), full.names = T)
        gWB[[i]][[j]] <- lapply(fnames[[i]][[j]], readRDS)
    }
    
    x <- all.equal(sapply(gWB[[i]][[1]], graph_attr, 'name'),
...
write_rds(gWB, paste0(savedir1, 'gWB.rds'))
write_rds(g.groupWB, paste0(savedir1, 'g.groupWB.rds'))
