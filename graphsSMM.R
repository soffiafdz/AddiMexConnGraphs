...
A.norm.sub <- matsSMM$A.norm.sub
A.norm.mean <- matsSMM$A.norm.mean
atlas <- "pSMM"
g.groupSMM <- gSMM <- fnames <- vector('list', length=length(groups))
...
for (i in seq_along(groups)) {
    ...
    g.groupSMM[[i]] <- lapply(seq_along(thresholds), function(x) 
        graph_from_adjacency_matrix(A.norm.mean[[x]][[i]], 
                                    mode = 'undirected', diag = F, weighted = T))
    
    for (x in seq_along(thresholds)) {V(g.groupSMM[[i]][[x]])$name <- as.character(pSMM$name)}
    
    g.groupSMM[[i]] <- llply(seq_along(thresholds), function(x) 
        set_brainGraph_attr_sof(g.groupSMM[[i]][[x]], atlas, modality = 'fmri', 
...
for (i in seq_along(groups)) {
    gSMM[[i]] <- fnames[[i]] <- vector('list', length = length(thresholds))
    for (j in seq_along(thresholds)) {
        fnames[[i]][[j]] <- list.files(savedir, 
                                       sprintf('*g%i_thr%02i.*', i, j), full.names = T)
        gSMM[[i]][[j]] <- lapply(fnames[[i]][[j]], readRDS)
    }
    
    x <- all.equal(sapply(gSMM[[i]][[1]], graph_attr, 'name'),
...
write_rds(gSMM, paste0(savedir1, 'gSMM.rds'))
write_rds(g.groupSMM, paste0(savedir1, 'g.groupSMM.rds'))
