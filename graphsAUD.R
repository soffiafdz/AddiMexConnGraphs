...
A.norm.sub <- matsAUD$A.norm.sub
A.norm.mean <- matsAUD$A.norm.mean
atlas <- "pAUD"
g.groupAUD <- gAUD <- fnames <- vector('list', length=length(groups))
...
for (i in seq_along(groups)) {
    ...
    g.groupAUD[[i]] <- lapply(seq_along(thresholds), function(x) 
        graph_from_adjacency_matrix(A.norm.mean[[x]][[i]], 
                                    mode = 'undirected', diag = F, weighted = T))
    
    for (x in seq_along(thresholds)) {V(g.groupAUD[[i]][[x]])$name <- as.character(pAUD$name)}
    
    g.groupAUD[[i]] <- llply(seq_along(thresholds), function(x) 
        set_brainGraph_attr_sof(g.groupAUD[[i]][[x]], atlas, modality = 'fmri', 
...
for (i in seq_along(groups)) {
    gAUD[[i]] <- fnames[[i]] <- vector('list', length = length(thresholds))
    for (j in seq_along(thresholds)) {
        fnames[[i]][[j]] <- list.files(savedir, 
                                       sprintf('*g%i_thr%02i.*', i, j), full.names = T)
        gAUD[[i]][[j]] <- lapply(fnames[[i]][[j]], read_rds)
    }
    
    x <- all.equal(sapply(gAUD[[i]][[1]], graph_attr, 'name'),
...
write_rds(gAUD, paste0(savedir1, 'gAUD.rds'))
write_rds(g.groupAUD, paste0(savedir1, 'g.groupAUD.rds'))
