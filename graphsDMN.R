...
A.norm.sub <- matsDMN$A.norm.sub
A.norm.mean <- matsDMN$A.norm.mean
atlas <- "pDMN"
g.groupDMN <- gDMN <- fnames <- vector('list', length=length(groups))
...
for (i in seq_along(groups)) {
    ...
    g.groupDMN[[i]] <- lapply(seq_along(thresholds), function(x) 
        graph_from_adjacency_matrix(A.norm.mean[[x]][[i]], 
                                    mode = 'undirected', diag = F, weighted = T))
    
    for (x in seq_along(thresholds)) {V(g.groupDMN[[i]][[x]])$name <- as.character(pDMN$name)}
    
    g.groupDMN[[i]] <- llply(seq_along(thresholds), function(x) 
        set_brainGraph_attr_sof(g.groupDMN[[i]][[x]], atlas, modality = 'fmri', 
...
for (i in seq_along(groups)) {
    gDMN[[i]] <- fnames[[i]] <- vector('list', length = length(thresholds))
    for (j in seq_along(thresholds)) {
        fnames[[i]][[j]] <- list.files(savedir, 
                                       sprintf('*g%i_thr%02i.*', i, j), full.names = T)
        gDMN[[i]][[j]] <- lapply(fnames[[i]][[j]], readRDS)
    }
    
    x <- all.equal(sapply(gDMN[[i]][[1]], graph_attr, 'name'),
...
write_rds(gDMN, paste0(savedir1, 'gDMN.rds'))
write_rds(g.groupDMN, paste0(savedir1, 'g.groupDMN.rds'))
