...
A.norm.sub <- matsSUB$A.norm.sub
A.norm.mean <- matsSUB$A.norm.mean
atlas <- "pSUB"
g.groupSUB <- gSUB <- fnames <- vector('list', length=length(groups))
...
for (i in seq_along(groups)) {
    ...
    g.groupSUB[[i]] <- lapply(seq_along(thresholds), function(x) 
        graph_from_adjacency_matrix(A.norm.mean[[x]][[i]], 
                                    mode = 'undirected', diag = F, weighted = T))
    
    for (x in seq_along(thresholds)) {V(g.groupSUB[[i]][[x]])$name <- as.character(pSUB$name)}
    
    g.groupSUB[[i]] <- llply(seq_along(thresholds), function(x) 
        set_brainGraph_attr_sof(g.groupSUB[[i]][[x]], atlas, modality = 'fmri', 
...
for (i in seq_along(groups)) {
    gSUB[[i]] <- fnames[[i]] <- vector('list', length = length(thresholds))
    for (j in seq_along(thresholds)) {
        fnames[[i]][[j]] <- list.files(savedir, 
                                       sprintf('*g%i_thr%02i.*', i, j), full.names = T)
        gSUB[[i]][[j]] <- lapply(fnames[[i]][[j]], readRDS)
    }
    
    x <- all.equal(sapply(gSUB[[i]][[1]], graph_attr, 'name'),
...
write_rds(gSUB, paste0(savedir1, 'gSUB.rds'))
write_rds(g.groupSUB, paste0(savedir1, 'g.groupSUB.rds'))
