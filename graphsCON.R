...
A.norm.sub <- matsCON$A.norm.sub
A.norm.mean <- matsCON$A.norm.mean
atlas <- "pCON"
g.groupCON <- gCON <- fnames <- vector('list', length=length(groups))
...
for (i in seq_along(groups)) {
    ...
    g.groupCON[[i]] <- lapply(seq_along(thresholds), function(x) 
        graph_from_adjacency_matrix(A.norm.mean[[x]][[i]], 
                                    mode = 'undirected', diag = F, weighted = T))
    
    for (x in seq_along(thresholds)) {V(g.groupCON[[i]][[x]])$name <- as.character(pCON$name)}
    
    g.groupCON[[i]] <- llply(seq_along(thresholds), function(x) 
        set_brainGraph_attr_sof(g.groupCON[[i]][[x]], atlas, modality = 'fmri', 
...
for (i in seq_along(groups)) {
    gCON[[i]] <- fnames[[i]] <- vector('list', length = length(thresholds))
    for (j in seq_along(thresholds)) {
        fnames[[i]][[j]] <- list.files(savedir, 
                                       sprintf('*g%i_thr%02i.*', i, j), full.names = T)
        gCON[[i]][[j]] <- lapply(fnames[[i]][[j]], readRDS)
    }
    
    x <- all.equal(sapply(gCON[[i]][[1]], graph_attr, 'name'),
...
write_rds(gCON, paste0(savedir1, 'gCON.rds'))
write_rds(g.groupCON, paste0(savedir1, 'g.groupCON.rds'))
